...
class Scheduler(object):
    def __init__(self, node_id, config={}, proxies=[]):
        ...
        if proxies and len(proxies) > 0:
            self.proxy_list = proxy_checker(proxies) if self.config['check_proxies'] else format_proxy_list(proxies)
            logger.info("number of live proxies: %d" % (len(self.proxy_list)))
            # each process only get one apikey...  if there are more proxies than apikeys,
            ...
            # if there are more apikeys than proxies, then each process will get a list of proxies,
