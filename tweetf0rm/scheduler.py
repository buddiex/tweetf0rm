...
class Scheduler(object):
    ...
    def __init__(self, node_id, config={}, proxies=[], check_proxy=False):
        self.node_id = node_id
        self.config = config
        if (proxies and len(proxies) > 0):
            self.proxy_list = proxy_checker(proxies) if check_proxy else format_proxy_list(proxies)
            ...
            # if there are more apikeys than proxies, then each process will get a list of proxies, and the process will restart it self if a proxy failed, and try the next available proxy
        ...
        logger.info("number of crawlers created: %d "%(number_of_processes))
    ...
    def new_crawler(self, node_id, apikeys, config, crawler_proxies = None):
        ...
        crawler = TwitterCrawler(node_id, crawler_id, copy.copy(apikeys), handlers=[create_handler(handler)], redis_config=copy.copy(config['redis_config']), proxies=crawler_proxies)
